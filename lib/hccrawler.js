...
class HCCrawler extends EventEmitter {
  ...
  async _followLinks(urls, options, depth) {
    ...
    //return undefined for skipped request
    var requests = await Promise.all(map(urls, async (url) => {
        var _options = extend({}, options, { url });
    ...
    await Promise.all(map(requests, async (req, index) => {
      if(req.hasOwnProperty("maxPages") && index > req.maxPages) return;
