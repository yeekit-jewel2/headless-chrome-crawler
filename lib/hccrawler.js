...
class HCCrawler extends EventEmitter {
  ...
  async wasBrowserKilled(browser) {
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    if (this.isBrowserConnected == false && (this._browser === false || this.wasBrowserKilled(this._browser))) {
      this._browser = await Puppeteer.launch(pick(options, LAUNCH_OPTIONS));
      console.log("resurecting browser");
    }
  ...
  _checkAllowedDomains(options) {
    ...
    if (options.allowedUrlName && !checkUrlName(options.allowedUrlName, hostname)) return false;
  ...
  async _followLinks(urls, options, depth) {
    if (depth >= options.maxDepth) {
      this.emit(HCCrawler.Events.MaxDepthReached, options);
      return;
    }
    var requestWithSameDomain = [];

    await Promise.all(map(urls, async (url) => {
        const _options = extend({}, options, { url });
        if(_options.hasOwnProperty("screenshot")){
            delete _options['screenshot'];
        }
        const skip = await this._skipRequest(_options);
        if (skip) return;
        requestWithSameDomain.push(_options);
    }));

    if (options.hasOwnProperty("maxPage") && requestWithSameDomain.length >= options.maxPage-1) {
        //we only limit maxpages -1 (1 is the parent url)
        requestWithSameDomain = requestWithSameDomain.slice(0, options.maxPage-1);
    }

    await Promise.all(map(requestWithSameDomain, async (req, index) => {
      await this._push(req, depth + 1, options.url);
    }));


    this.emit(HCCrawler.Events.ChildRequestsAdded, requestWithSameDomain, options);
  }
...
HCCrawler.Events = {
  ...
  ChildRequestsAdded: 'childrequestsadded'
};
