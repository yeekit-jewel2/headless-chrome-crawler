...
class HCCrawler extends EventEmitter {
  ...
  executePromiseWithinMs(promise, ms) {
  ...
  async _request(options, depth, previousUrl, retryCount = 0) {
    ...
    const pid = crawler.page().browser().process().pid;
    options.pid = pid;
    try {
      const res = await this._crawl(crawler);
      await this.executePromiseWithinMs(crawler.close(), 10000);
      ...
      try {
        ...
        console.log("Forcefully kill browser");
      }
