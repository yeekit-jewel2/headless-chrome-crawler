...
class HCCrawler extends EventEmitter {
  ...
  async _followLinks(urls, options, depth) {
    ...
    await Promise.all(map(urls, async url => {
      ...
      if (_options.hasOwnProperty("maxPages")) {
          _options.pagesScraped = _options.pagesScraped -1;
      }
      else {
          _options.pagesScraped = _options.maxPages -1;
      }
      const skip = await this._skipRequest(_options);
      if (skip) return;
      if (_options.pagesScraped == 0) return;
